...
# Load embeddings model (sentence-transformers via transformers pipeline)
try:
    embed_pipe = pipeline("feature-extraction", model="sentence-transformers/all-MiniLM-L6-v2")
except Exception as e:
    print("⚠️ jobs.utils: embedding model load error:", e)
    embed_pipe = None
...
def _to_vec(text):
    if not text or not embed_pipe:
        return np.zeros((1,384))
    vecs = embed_pipe(text[:512])[0]  # list of token vectors
    # average token vectors to one vector
...
def calculate_match_score(job_skills: str, resume_skills: str, resume_experience: str) -> int:
    ...
    # keyword overlap
    job_keywords = [s.strip().lower() for s in job_skills.split(',') if s.strip()]
    user_skills = [s.strip().lower() for s in resume_skills.split(',') if s.strip()]
    matched = set(job_keywords) & set(user_skills)
    keyword_score = (len(matched) / len(job_keywords)) * 100 if job_keywords else 0

    # semantic similarity
    job_text = " ".join(job_keywords)
    resume_text = resume_skills + " " + (resume_experience or "")
    job_vec = _to_vec(job_text)
    resume_vec = _to_vec(resume_text)
    try:
        sem = float(cosine_similarity(job_vec, resume_vec)[0][0])  # -1..1
        semantic_score = max(0, (sem + 1) / 2) * 100  # normalize to 0..100
    except Exception:
        semantic_score = 0

    exp_bonus = 10 if resume_experience and len(resume_experience) > 50 else 0

    final = (keyword_score * 0.5) + (semantic_score * 0.4) + exp_bonus
    return int(min(100, round(final)))
